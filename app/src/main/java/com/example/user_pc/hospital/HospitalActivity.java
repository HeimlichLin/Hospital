        ...
        client.get(urlString, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                for(int i=0; i<records.length(); i++){
                    ...
                    mSearchList.add(board.optString("depday") + "                  " + "看診時間：" + board.optString("noon") + "\n"
                            + "看診院區：" + board.optString("area") + "   " + "科別名稱：" + board.optString("depname") + "\n"
                            + "診間代碼：" + board.optString("room") +  "                " + "醫師姓名：" + board.optString("docname"));
                }
            }
